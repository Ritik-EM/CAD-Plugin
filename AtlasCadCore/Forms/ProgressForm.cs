...
namespace AtlasCadCore.Forms
{
    public class ProgressForm : Form
    {
        ...
        public ProgressForm(string title)
        {
            ...
            _phaseLabel = new Label { Location = new Point(15, 20), Width = 420, Height = 25, Text = "Starting…" };
            Controls.Add(_phaseLabel);

            _bar = new ProgressBar
            {
                Location = new Point(15, 50), Width = 420, Height = 20,
                Style = ProgressBarStyle.Continuous, Minimum = 0, Maximum = 100, Value = 0,
            };
            Controls.Add(_bar);

            _cancelButton = new Button { Text = "Cancel", Location = new Point(350, 85), Width = 85 };
        ...
        {
            if (InvokeRequired) { BeginInvoke(new Action(() => SetPhase(label, current, total))); return; }
