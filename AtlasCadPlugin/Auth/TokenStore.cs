...
namespace AtlasCadPlugin.Auth
{
    public class StoredToken
    {
        ...
        // Unix seconds. JWT exp claim — same source of truth as atlas-api.
        public long ExpiresAt { get; set; }

        // 60s leeway covers clock skew between this machine and atlas-api.
    ...
    /// <summary>
    /// Persists the JWT to %APPDATA%\AtlasCad\token.dat, encrypted with
    /// Windows DPAPI scoped to the current user. Only the same Windows account
    /// on the same machine can decrypt — losing the file or copying it to
    /// another machine forces a re-login.
    /// </summary>
    public static class TokenStore
    {
        ...
        private static StoredToken Load()
        {
            ...
            {
                // Corrupt / copied from another machine / DPAPI key rotated.
                // Treat as no token rather than crash the plugin.
                return null;
            }
        }
    }
