...
namespace AtlasCadPlugin.Auth
{
    ...
    /// <summary>
    /// Talks to Euler's central auth (octopus) to exchange username+password
    /// for a JWT. The same JWT is then sent to atlas-api as Authorization: Bearer
    /// — atlas-api's AuthMiddleware verifies it against the shared HS256 secret.
    /// </summary>
    public class AuthService
    {
        ...
        {
            // Octopus expects the password SHA-256-hashed at the client side
            // (matches atlas-ui's generateSHA256Hash in src/lib/utils.ts).
